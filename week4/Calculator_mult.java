...
public class Calculator_mult {
    ...
    static ArrayList<Number> multover(ArrayList<Number> array){
        int index = 0;
        int i=1;
        while(i < array.size()){
            if(array.get(i).key == "*"){
                array.set(i - 1, array.get(i - 1).value * array.get(i + 1).value);
                cut(array, i, i+1);
            }
            else if(array.get(i).key == "/"){
                array.value.set(i - 1, array.get(i - 1).value / array.get(i + 1).value);
                cut(array, i, i+1);
            }
        }
        return array;
